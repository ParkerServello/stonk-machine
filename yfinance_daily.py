...
def update_raw_data(ticker):
    ...
    end_date =  str(dt.datetime.date(dt.datetime.now()))
    ...
    # remove duplicates, yahoo returns an extra row for yesterday if ran before market  
    ...
    if initializing:
        ...
        data.append(new_data).to_csv(raw_data_path, index = False)
